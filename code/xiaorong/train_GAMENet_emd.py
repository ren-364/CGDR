...
from models_emd import GAMENet
...
def main():
    ...
    voc_size = (len(diag_voc.idx2word), len(pro_voc.idx2word), len(med_voc.idx2word))
    model = GAMENet(voc_size, ehr_adj, diag2med, ddi_adj, emb_dim=128, device=device, ddi_in_memory=DDI_IN_MEM, d_Hetertensor=d_Hetertensor, p_Hetertensor=p_Hetertensor)
    if TEST:
