...
import numpy as np
from dnc import DNC
from layers import GraphConvolution, DM_gcn, DDI_gcn
...
class CGMR(nn.Module):
    def __init__(self, voc, vocab_size, ddi_Hetertensor, diag2med, ddi_adj, emb_dim, device=torch.device('cpu:0'), ddi_in_memory=True, d_Hetertensor=None, p_Hetertensor=None):
        ...
        self.MultiHead = nn.ModuleList([MultiHeadAttention(emb_dim * 2, num_heads=4, dropout=0.4) for _ in range(2)])
        ...
        self.output = nn.Sequential(
            nn.ReLU(),
            nn.Linear(emb_dim * 8, emb_dim * 6),
            nn.ReLU(),
            nn.Linear(emb_dim * 6, vocab_size[2])
        )
        self.ddi_gcn = DDI_gcn(mednum=vocab_size[2],featuredim=emb_dim * 2)

        self.embedding = DM_gcn(
            Diagnum=vocab_size[0],
            mednum=vocab_size[2],
            pronum=vocab_size[1],
            featuredim=emb_dim * 2
        )

        self.d_Hetertensor = d_Hetertensor
        self.p_Hetertensor = p_Hetertensor
        self.ddi_Hetertensor = ddi_Hetertensor
    ...
    def forward(self, input):
        ...
        ddiEmd = self.ddi_gcn(self.ddi_Hetertensor)
        medEmd = medEmd_dp - ddiEmd * self.inter
        i1_seq = []
        i2_seq = []
        #i3_seq = []
        ...
        for adm in input:
            i1 = mean_embedding(diagEmd[adm[0]].unsqueeze(dim=0)) # (1,1,dim)
            i2 = mean_embedding(proEmd[adm[1]].unsqueeze(dim=0))
        ...
        output = self.output(torch.cat([r1, r2, fact1, fact2], dim=-1)) # (1, dim)
