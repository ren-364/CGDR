...
def eval(model, data_eval, voc_size, epoch):
    ...
    for step, input in enumerate(data_eval):
        ...
        case_study[step] = {'ja': adm_ja, 'patient': input, 'y_label': y_pred_label, 'ddi_rate': current_ddi_rate, 'real_ddi':real_ddi}
    ...
    dill.dump(case_study, open(os.path.join('saved', model_name, 'CGMR.pkl'), 'wb'))

    # print('avg med', med_cnt / visit_cnt)

    return ddi_rate, np.mean(ja), np.mean(prauc), np.mean(avg_p), np.mean(avg_r), np.mean(avg_f1)

def data_ddi(data, voc_size):
    smm_record = []
    for step, input in enumerate(data):
        ...
        for adm_idx, adm in enumerate(input):
            ...
            y_gt_tmp = np.zeros(voc_size[2])
            y_gt_tmp[adm[2]] = 1
            y_gt_tmp_label_tmp = np.where(y_gt_tmp == 1)[0]
            y_pred_label.append(sorted(y_gt_tmp_label_tmp))
            smm_record.append(y_pred_label)
...
def main():
    ...
    model = CGMR(voc, voc_size, ddi_Hetertensor, diag2med, ddi_adj, emb_dim=64, device=device, ddi_in_memory=DDI_IN_MEM, d_Hetertensor=d_Hetertensor, p_Hetertensor=p_Hetertensor)
    if TEST:
        model.load_state_dict(torch.load(open('F:\projects\CGDR\code\Epoch_12_JA_0.5655_DDI_0.0688.model', 'rb')))
    ...
    if TEST:
        ...
        for epoch in range(EPOCH):
            ...
            for step, input in enumerate(data_train):
                for idx, adm in enumerate(input):
                    ...
                    target_output1, batch_neg_loss, loss_ddi = model(seq_input)
                    ...
                    if current_ddi_rate <= TARGET_DDI:
                        ...
                        beta = args.coef * (1 - (current_ddi_rate / TARGET_DDI))
                        beta = min(math.exp(beta), 1)
                        loss = beta * (0.95 * loss1 + 0.05 * loss3) + (1 - beta) * loss_ddi
                    optimizer.zero_grad()
        ...
        dill.dump(history['ja'], open(os.path.join('saved', model_name, 'jaccard_history.pkl'), 'wb'))
